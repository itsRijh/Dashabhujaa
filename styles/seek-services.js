    ...
    .then(res => {
        ...
        if (res.message) {
            userDetails = userDetails + `
                  <img src=${res.profile_pic} class="buy-prod-profile">
                  <div style="color: black;"> ${res.name} </div>`
        }
...
function generateModalStuff(res) {
                        ...
                        <div class="col-12 col-lg-8">
                            ...
                            <div class="row" style="padding-bottom: 20px;">
                                ...
                                <div class="col-lg-6 col-12" style="text-align: center;">
                                    <button onclick="registerInterest(event)" id="${res[i].SERVICE_ID}" data-pid=${res[i].SERVICE_ID} class="btn interest-modal-btn">Interested</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    document.getElementById("seek-services").innerHTML = content;
}

document.getElementById("prodSearch").addEventListener("input", () => {

    fetch("http://localhost:3500/services/search?q=" + document.getElementById("prodSearch").value)
        .then((response) => response.json())
        .then((res) => {
            res = res.message;
...
fetch("http://localhost:3500/services/getall", {
    method: "GET",
    headers: {
...
function registerInterest(event) {
  ...
  document.getElementById(`interest-cover-${prodId}`).appendChild(div);
    ...
    fetch('http://localhost:3500/services/interested', {
        method: 'POST',
        headers: {
            "Content-Type": "application/json",
            Authorization: token,
